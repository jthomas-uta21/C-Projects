...
Vector2 get_position(int team) {
    if (team) {
        return (Vector2){ 0 * uis.screen_width, 0 * uis.screen_height};
    } else {
...
void init_game() {
    ...
    entities[0].pos.x = 0;
    entities[0].pos.y = 0;
    entities[0].texture = LoadTexture("/Users/johnthomas/Documents/CSE 1320/monster_battle-main");
    printf("[DEBUG] Texture %d\n", entities[0].texture.id);

    printf("[DEBUG] Loadedn the texture and player\n");
